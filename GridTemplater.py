...
try:
    ...
    exit('This example requires that `svgwrite` library'
...
def convertpxtomm(pixels, dpi=132):
    """Converts pixels to mm. Thanks sijudge for this"""
    spixels_mm = (float(pixels)*25.4)/dpi
    return spixels_mm

def get_device_details(type):
    """Looks up some details to help minimise
        lots of options being pushed to the script"""
    tree = ET.parse(__location__+"/templates/Type_"+type+".xml")
    doc = tree.getroot()
    return {'winwidth':int(doc.find('winwidth').text),
            'winheight':int(doc.find('winheight').text) ,
            'dpi': int(doc.find('dpi').text)}

def create_grid(w, h, x, y, cellspacing=8, spacingunits='px',
            rxy=4, fname='output', linewidth=0.01, dpi=132):
    """A rather hefty function that does the hard work"""
    # ok lets do some math!
    # whats the cell spacing?
    # NB: this is mm - in the grid its pixels so  this needs converting
    ...
    dwg = svgwrite.Drawing(filename = fname+".svg",
                          size = (str(w)+"mm", str(h)+"mm"))
...
# ./GridTemplater.py --cellwidth 7 --cellheight 5 --winwidth 198
#   --winheight 148 --cellspacing 1.705  or ./GridTemplater.py --type iPad
#    --cellwidth 7 --cellheight 5 --cellspacing 8 -u px
...
if __name__ == '__main__':
    ...
    if arguments['--type'] != None:
        typedets = get_device_details(arguments['--type'])
...
create_grid(arguments['--winwidth'], arguments['--winheight'],
            arguments['--cellwidth'], arguments['--cellheight'], arguments['--cellspacing'],
            arguments['--spacingunits'], arguments['--rxy'], arguments['--filename'],
            arguments['--linewidth'], arguments['--dpi'])
